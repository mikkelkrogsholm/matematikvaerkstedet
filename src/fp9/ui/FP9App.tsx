...
export function FP9App(){
 ...
 const [aiBusy,setAiBusy]=useState(false);const animationTimer=useRef<number|null>(null);const [renderRetry,setRenderRetry]=useState(0);
 ...
 const mutate=async(action:LocalAction):Promise<AttemptView|undefined>=>{
  ...
  if(action.type!=='animation-next'&&animationTimer.current!==null){window.clearTimeout(animationTimer.current);animationTimer.current=null;}
  ...
  const run=async()=>{
   ...
   if(action.type==='animation-next'&&current.scenes[action.taskId]?.animation?.steps[0]?.command.actionId!==action.commandId){failedSaves.current.delete(key);setError('');setAttempt(current);return current;}
 ...
 const showHome=async()=>{if(!await flush())return;const current=attemptRef.current;if(current&&Object.values(current.scenes).some(scene=>scene.animation||scene.pendingRender)){if(!await mutate({type:'animation-stop',taskId:current.activeTaskId}))return;}try{const result=await request<{attempts:AttemptSummary[]}>('/api/fp9/attempts');setAttempts(result.attempts);setScreen('setup');}catch(e){setError(String(e));}};
 ...
 // Auto-play yields between confirmed steps; reduced motion uses explicit Next.
 useEffect(()=>{
  if(screen!=='work'||!attempt||!active||save!=='saved')return;
  const scene=attempt.scenes[active.id];
  if(!scene?.animation||scene.pendingRender||window.matchMedia('(prefers-reduced-motion: reduce)').matches)return;
  const timer=window.setTimeout(()=>void mutate({type:'animation-next',taskId:active.id,commandId:scene.animation!.steps[0]!.command.actionId}),1000);animationTimer.current=timer;
  return()=>{window.clearTimeout(timer);animationTimer.current=null;};
 },[attempt?.revision,screen,save]);
  ...
  let cancelled=false;const frame=requestAnimationFrame(()=>{
   const run=async()=>{
    ...
    try{
     ...
     setError(e instanceof Error?e.message:'Forklaringslaget kunne ikke bekræftes.');
     try{const fresh=await request<AttemptView>(`/api/fp9/attempts/${attempt.id}`);if(attemptRef.current?.id===fresh.id&&attemptRef.current.revision<=fresh.revision){attemptRef.current=fresh;setAttempt(fresh);if(fresh.scenes[active.id]?.pendingRender?.token!==pending.token)setError('');}}catch{}
     return undefined;
    }
   };
   queue.current=queue.current.then(run,run);
  });return()=>{cancelled=true;cancelAnimationFrame(frame);};
 },[attempt?.id,attempt?.revision,active?.id,renderRetry]);
