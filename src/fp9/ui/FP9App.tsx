...
function DraftAnswer({question:q,initial,locked,onSave,feedback,allowFeedback,onFeedback}:{question:PublicTask['questions'][number];initial:Answer;locked:boolean;onSave:(a:Answer)=>void;feedback:AttemptView['assessments'][string][string]|undefined;allowFeedback:boolean;onFeedback:()=>void}){
 ...
 useEffect(()=>{if(q.answerKind==='geometry')setDraft(initial);},[JSON.stringify(initial.points)]);
 const change=(patch:Partial<Answer>)=>{const next={...draft,...patch};setDraft(next);onSave(next);};
 ...
 <label>Dit svar{q.unit?` (${q.unit})`:''}{q.answerKind==='reasoning'?<textarea disabled={locked} maxLength={4000} value={draft.text} onChange={e=>change({text:e.target.value})}/>:<input disabled={locked} maxLength={4000} inputMode={q.answerKind==='number'&&!['fraction','scientific'].includes(q.format??'')?'decimal':'text'} value={draft.text} onChange={e=>change({text:e.target.value})}/>}</label>
 {q.answerKind!=='reasoning'&&<label>Forklar din metode (når opgaven beder om det)<textarea disabled={locked} maxLength={4000} value={draft.explanation??''} onChange={e=>change({explanation:e.target.value})}/></label>}</>}
...
function DraftNote({initial,locked,onSave}:{initial:string;locked:boolean;onSave:(text:string)=>void}){const [text,setText]=useState(initial);return <label className="fp9-note">Dine noter<textarea disabled={locked} maxLength={4000} value={text} onChange={e=>{setText(e.target.value);onSave(e.target.value);}}/></label>;}
...
function CoordinateField({value,disabled,onSave}:{value:number;disabled:boolean;onSave:(value:string)=>void}){
 ...
 const commit=()=>{const normalized=text.trim().replace(',','.');if(!/^[+-]?(?:\d+(?:\.\d+)?|\.\d+)$/.test(normalized)||!Number.isFinite(Number(normalized))){setInvalid(true);return;}setInvalid(false);onSave(normalized);};
