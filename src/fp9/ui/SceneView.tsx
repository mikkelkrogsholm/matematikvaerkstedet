...
export function SceneView({task,state,withAids,locked,onStudent}:Props){
 ...
 const visibleGraph=scene.kind==='grid'||scene.kind==='shape'||(scene.kind==='data'&&!!scene.categories)||(scene.kind!=='text'&&withAids)||ai.some(o=>o.visible);
 ...
 onPointerUp={()=>{if(drag.current){const last=drag.current;save(students.map(o=>o.id===last.id?{...o,x:last.x,y:last.y}:o),[last.id]);drag.current=null;}}} onPointerCancel={()=>{drag.current=null;setDraft(null);}}>
 {scene.kind!=='shape'&&<g>{ticks(x).map(n=><g key={`x${n}`}><line className="grid" x1={px(n)} x2={px(n)} y1="0" y2="100"/><text x={px(n)} y="105" textAnchor="middle">{scene.kind==='data'&&scene.categories?scene.categories[n-1]:n.toLocaleString('da-DK',{maximumFractionDigits:2})}</text></g>)}
 ...
 <text x="50" y="113" textAnchor="middle">{x.label} {x.unit?`(${x.unit})`:''}</text><text x="0" y="-5">{y.label} {y.unit?`(${y.unit})`:''}</text></g>}
 {scene.kind==='shape'&&<g className="given-shape">{scene.givens.edges.map(([a,b],i)=>{const p=scene.givens.points.find(p=>p.id===a)!,q=scene.givens.points.find(p=>p.id===b)!;return <line key={i} x1={px(p.x)} y1={py(p.y)} x2={px(q.x)} y2={py(q.y)} stroke="#34765b" strokeWidth=".7"/>;})}{scene.givens.labels.map((p,i)=><text key={i} x={px(p.x)} y={py(p.y)} textAnchor="middle">{p.text}</text>)}</g>}
 ...
 {scene.kind==='data'&&scene.categories&&scene.data[0]!.values.map((v,i)=><rect key={i} x={px(i+1)-4} width="8" y={py(v)} height={py(0)-py(v)} fill="#34765b"/>)}
 {scene.kind==='data'&&!scene.categories&&withAids&&scene.data.flatMap((row,j)=>row.values.map((v,i)=><g key={`${j}-${i}`}><circle cx={px(i+1)} cy={py(v)} r={j?1.8:1} fill={j?'none':'#34765b'} stroke={j?'#b47a35':'#34765b'} strokeWidth=".5"/><title>{`${row.label}: ${v}`}</title></g>))}
 ...
 {scene.kind==='grid'&&scene.givens.points.length===2&&<line x1={px(scene.givens.points[0]!.point.x)} y1={py(scene.givens.points[0]!.point.y)} x2={px(scene.givens.points[1]!.point.x)} y2={py(scene.givens.points[1]!.point.y)} stroke="#52665a" strokeWidth=".5"/>}
 {task.familyId==='F13'&&task.variant<2&&students.length>=3&&(()=>{const center={x:students.reduce((s,p)=>s+Number(p.x),0)/students.length,y:students.reduce((s,p)=>s+Number(p.y),0)/students.length};const ordered=[...students].sort((a,b)=>Math.atan2(Number(a.y)-center.y,Number(a.x)-center.x)-Math.atan2(Number(b.y)-center.y,Number(b.x)-center.x));return <polygon points={ordered.map(p=>`${px(Number(p.x))},${py(Number(p.y))}`).join(' ')} fill="#376ba61a" stroke="#376ba6" strokeWidth=".5"/>;})()}
 {students.map(o=>typeof o.x==='number'&&typeof o.y==='number'?<g key={o.id}><circle data-student-object-id={o.id} className="student-point" cx={px(o.x)} cy={py(o.y)} r={selection.includes(o.id)?1.8:1.3} role="button" tabIndex={locked?-1:0} aria-label={describe(o)} onKeyDown={e=>{if(locked)return;if(e.key==='Enter'){select(o.id);return;}const dx=e.key==='ArrowLeft'?-1:e.key==='ArrowRight'?1:0,dy=e.key==='ArrowDown'?-1:e.key==='ArrowUp'?1:0;if(dx||dy){e.preventDefault();save(students.map(p=>p.id===o.id?{...p,x:Math.max(x.min,Math.min(x.max,Number(o.x)+dx)),y:Math.max(y.min,Math.min(y.max,Number(o.y)+dy))}:p),[o.id]);}}} onPointerDown={e=>{e.stopPropagation();if(locked)return;drag.current={id:o.id,x:Number(o.x),y:Number(o.y)};e.currentTarget.setPointerCapture(e.pointerId);}}/></g>:null)}
 ...
 {scene.kind==='data'&&withAids&&<p>● {scene.data[0]?.label}{scene.data[1]&&<>　○ {scene.data[1].label}</>}</p>}
 ...
 {selection.length>0&&<p aria-live="polite">Valgt: {students.filter(o=>selection.includes(o.id)).map(describe).join(', ')}</p>}
