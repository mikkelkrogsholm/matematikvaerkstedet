...
function App() {
  ...
  function selectMode(next: Mode) {
    setMode(next); setStep(0); setExercise(0); setAnswer(''); setFeedback(''); setInput('');
  ...
  function submit(event: React.FormEvent) {
    ...
    if (/hint|hjælp|opgave/.test(normalized)) say(task.hint);
    else if (/forklar|hvorfor|hvordan|procent|brøk|tangent|hældning/.test(normalized)) explain();
    else say('Denne prototype har en lokal guide, som kan forklare figuren og give hints. Prøv “forklar figuren” eller “giv mig et hint”. Fri AI-samtale er ikke tilkoblet endnu.');
  }
  ...
  return <div className="app-shell">
    ...
    <div className="workspace">
      ...
      <main className="main-panel min-w-0">
        ...
        {step===0 && <div className="step-footer"><p>Begynd med at ændre {junior?'antallet af dele.':'punktets position.'}</p><button className="primary" onClick={()=>{setStep(1);explain();}}>Se sammenhængen <span>→</span></button></div>}
      ...
      <aside className="guide-panel"><div className="guide-heading"><span className="guide-avatar">m.</span><div><h2>Din matematikguide</h2><small>Vi tager det ét skridt ad gangen</small></div></div><div className="demo-notice">Guidet demo · uden AI-forbindelse</div><div ref={messagePanel} className="messages" aria-live="polite">{messages.map((m,i)=><div className={`message ${m.role}`} key={i}><span>{m.role==='guide'?'GUIDEN':'DIG'}</span><p>{m.text}</p></div>)}</div><div className="guide-bottom"><span className="eyebrow">PRØV AT SPØRGE</span><button className="suggestion" onClick={explain}>Forklar det, jeg ser <span>↗</span></button><button className="suggestion" onClick={()=>{setStep(2);say(task.hint);}}>Giv mig et hint <span>↗</span></button><form className="chat-form" onSubmit={submit}><label htmlFor="question" className="sr-only">Spørg den lokale guide</label><input id="question" value={input} onChange={e=>setInput(e.target.value)} placeholder="Spørg til figuren…"/><button aria-label="Send spørgsmål" disabled={!input.trim()}>↑</button></form><small>Prøv dig frem. Her må du gerne tage fejl.</small></div></aside>
