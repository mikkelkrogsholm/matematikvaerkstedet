...
const review=await Bun.file(new URL('./review-manifest.json',import.meta.url)).json().catch(()=>null);
...
const samples=families.flatMap(family=>[101,202].flatMap(seed=>[0,1,2].map(variant=>{
 ...
 const reviewed=review?.cases?.[task.id]===createHash('sha256').update(JSON.stringify(publicTask(task))).digest('hex');
 return {familyId:family.id,seed,variant,examType,familyVersion:task.version,task:publicTask(task),reference:task.marking,review:{mathematicalChecks:'Covered by independent 100-seed tests; see generators.test.ts',visual:reviewed?'agent-reviewed':'pending',reviewer:reviewed?review.reviewer:null,date:reviewed?review.date:null}};
})));
